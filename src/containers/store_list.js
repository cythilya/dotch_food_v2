...
import { Container, Header as HeaderSemanticUI, Item, Grid, Icon, Sticky, Segment, Message, Card, Image, Button, Label, Loader, Divider } from 'semantic-ui-react';
...
class StoreList extends Component {
  ...
  fetchStoresData(keyword) {
    keyword ? this.props.fetchStoreListByTag(keyword) : this.props.fetchStoreList();
  }
    ...
    return (
      <div className={isEmpty ? 'show' : 'hide'}>
        <Message icon>
          ...
          <Message.Content>
            <Message.Header>找不到</Message.Header>
            <Link to='/' title='回「吃什麼」首頁'>回首頁</Link>或看熱門關鍵字
            <Link to={`/tags/${remainTags[0]}`}>{remainTags[0]}</Link>、
            <Link to={`/tags/${remainTags[1]}`}>{remainTags[1]}</Link>、
            <Link to={`/tags/${remainTags[2]}`}>{remainTags[2]}</Link>
          </Message.Content>
        </Message>
        <div>
          <HeaderSemanticUI as='h2'>猜你想吃</HeaderSemanticUI>
          <Card.Group itemsPerRow={3}>
            <Card>
              ...
              <Card.Content>
                ...
                <Card.Description>
                  <p>台北市和平東路二段175巷12號</p>
                  <p>巷弄中，簡單溫暖的小店，賣推薦的食材，還有簡單的料理。</p>
                </Card.Description>
              </Card.Content>
              <Card.Content extra>
                <Label>
            ...
            <Card>
              ...
              <Card.Content>
                ...
                <Card.Description>
                  <p>台北市通化街140-1號</p>
                  <p>小小不到九坪的三角窗，我們採半開放式空間，座位不多不少就十個，但卻可以為您帶來意想不到的美好滋味。 我們以天然的食材，給予客人最健康的享受，來迎接每一個早晨。</p>
                </Card.Description>
              </Card.Content>
              <Card.Content extra>
                <Label>
            ...
            <Card>
              <Image src='https://scontent.ftpe4-2.fna.fbcdn.net/v/t1.0-9/12391389_458639401011205_5087441327061549880_n.jpg?_nc_cat=0&oh=93452823d80f06352dcd1f12348bc3ac&oe=5B71F63A' />
              <Card.Content>
                <Card.Header>
                  YAYOI Taiwan (南京松江店)
                </Card.Header>
                ...
                <Card.Description>
                  <p>台北市中山區南京東路二段97號1樓</p>
                  <p>「YAYOI」是一家日式定食餐廳。提供營養均衡的「定食」，其中包括日本人的主食\u2015剛出鍋的金芽米飯、味噌湯、主菜和副菜等。「YAYOI」將傳承日本美食，並將其推向世界。</p>
                </Card.Description>
              </Card.Content>
              <Card.Content extra>
                <Label>
